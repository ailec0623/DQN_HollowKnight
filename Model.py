...
class Model:
   ...
   def _build_model(self):
       ...
       model.add(Conv2D(16, 5,strides=3, input_shape=self.input_shape, padding="SAME", kernel_initializer=init, activation='relu', name='c1'))
       model.add(BatchNormalization(name='b1'))
       model.add(MaxPooling2D(pool_size=2, strides=2, padding="VALID", name='p1'))
       model.add(Conv2D(32, 5,strides=2, padding="SAME", kernel_initializer=init, activation='relu', name='c2'))
       model.add(BatchNormalization(name='b2'))
       model.add(MaxPooling2D(pool_size=2, strides=2, padding="VALID", name='p2'))
       model.add(Conv2D(64, 3,name='c3', padding="SAME", kernel_initializer=init, activation='relu') )
       ...
       target_model.add(Conv2D(16, 5,strides=3, input_shape=self.input_shape, padding="SAME", kernel_initializer=init, activation='relu', name='c1'))
       target_model.add(BatchNormalization(name='b1'))
       target_model.add(MaxPooling2D(pool_size=2, strides=2, padding="VALID", name='p1'))
       target_model.add(Conv2D(32, 5,strides=2, padding="SAME", kernel_initializer=init, activation='relu', name='c2'))
       target_model.add(BatchNormalization(name='b2'))
       target_model.add(MaxPooling2D(pool_size=2, strides=2, padding="VALID", name='p2'))
       target_model.add(Conv2D(64, 3,name='c3', padding="SAME", kernel_initializer=init, activation='relu') )
